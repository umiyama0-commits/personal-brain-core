...
async def expand_query_structured(query: str, max_keywords: int = 6) -> dict:
    ...
    prompt = (
        ...
        "例 5: 「来週月曜日の新店会議の議題のサンプルリンクって何?」\n"
        "  意図: 「サンプルリンク」 という固有名詞 (会社名/案件名/PJ名) が何かを知りたい。\n"
        "        「新店会議」「議題」「来週月曜」 は その固有名詞に出会った **状況説明** であって\n"
        "        検索対象そのものではない。\n"
        "  → [\"サンプルリンク\", \"サンプルリンク 出店\", \"サンプルリンク PJ\", \"新規出店\", \"新店 出店\"]\n"
...
async def search_drive_semantic(
    ...
    # ★2026-06-07: 旧 keywords[:3] だと、LLM が固有名詞 (= サンプルリンク 等) を
